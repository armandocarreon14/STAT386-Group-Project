...
TIME_COLS = [
    ...
    "Transition 1 Time", "Transition 2 Time"
]
...
for c in TIME_COLS:
    if c in modified_dataset.columns:
        modified_dataset[c + " (sec)"] = modified_dataset[c].apply(parse_time_to_seconds)

#adding transition times and creating a new column
if "Transition 1 Time (sec)" in modified_dataset.columns and "Transition 2 Time (sec)" in modified_dataset.columns:
    modified_dataset["Transitions (sec)"] = modified_dataset["Transition 1 Time (sec)"] + modified_dataset["Transition 2 Time (sec)"]

#cleans data genders
if "gender" in modified_dataset.columns:
    modified_dataset["gender_norm"] = modified_dataset["gender"].apply(normalize_gender)

#some athletes have 0000 as timing. That means they didn't finish. We need to set athletes with an actual timing > 0
modified_dataset["Finished"] = modified_dataset["Overall Time (sec)"].notna() & (modified_dataset["Overall Time (sec)"] > 0)
if "Finish" in modified_dataset.columns:
    modified_dataset["Finished"] = modified_dataset["Finished"] & (modified_dataset["Finish"].astype(str).str.upper().str.strip() == "FIN")

fin_df = modified_dataset[modified_dataset["Finished"]].copy()
